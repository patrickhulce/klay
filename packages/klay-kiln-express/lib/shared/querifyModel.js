...
function generateModelsForProperty(name, model, options) {
  var modifiedModel = model.optional().default();
  var useEquality = options.equality === true || _.includes(options.equality, name);
  var useRange = options.range === true || _.includes(options.range, name);
  ...
  var selectorsModel = klay.builders.object().

...
function Model(spec) {
  ...
  _.forEach(assignments, function (value, name) {
    if (typeof Model.prototype[name] !== 'function') {
      throw Error('unknown property of Model: ' + name);
    }
...
Model.prototype._validateDefinedness = function (value) {
  ...
  if (isRequired) {
    assert.ok(!isUndefined, 'isUndefined');
    assert.ok(!isNull || isNullable, 'isNull');
    return value;
  } else if (!isNullable) {
    assert.ok(!isNull, 'isNull');
    return value;
  } else if (isUndefined && hasDefault) {
    return defaultValue;
  }
...
Model.prototype._validateValue = function (value) {
  ...
  if (validate instanceof RegExp) {
    ...
    assert.equal(typeof validate, 'function', 'typeof:validate');
  }
...
Model.prototype._validate = function (value, root, path) {
  try {
    assert.ok(this.spec.type, 'isUndefined:type');
    ...
    value = this._validateValue(value, path);
    if (value instanceof ValidationResult) {
      return value;
    } else {
      return new ValidationResult(value, true);
    ...
    e.valuePath = path;
    return new ValidationResult(value, false, e);
  }
};
